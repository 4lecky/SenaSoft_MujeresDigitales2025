...
<body>
    ...
    <div class="sectionIzquierda">
        ...
        <!-- <form action="{{ route('eventos.update', ['evento' => $evento->id_eventos]) }}" method="POST" class="container_form"> -->
        <form action="{{ route('eventos.update', $evento->id_eventos) }}" method="POST" class="container_form">
                @csrf
                @method('PUT')
